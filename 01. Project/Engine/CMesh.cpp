...
CMesh* CMesh::CreateFromContainer(CFBXLoader& _loader)
{
	...
	if (_loader.GetContainerCount() == 1)
	{
		...
		for (UINT i = 0; i < iVtxCount; ++i)
		{
			...
			// max : three uvs
			pSys[i].vUV0 = container->vecUVs[0][i];
			pSys[i].vUV1 = container->vecUVs[1][i];
			pSys[i].vUV2 = container->vecUVs[2][i];
	...
	else
	{
		...
		for (int containercount = 0; containercount < _loader.GetContainerCount(); ++containercount)
		{
			const tContainer* container = &_loader.GetContainer(containercount);

			iAllVtxCount += (UINT)container->vecPos.size();
		}

		pMesh->m_iVtxCount = iAllVtxCount;
		...
		for (int containercount = 0; containercount < _loader.GetContainerCount(); ++containercount)
		{
			tContainer* container = &_loader.GetContainer(containercount);

			UINT iVtxCount = (UINT)container->vecPos.size();
			
			for (UINT i = iAllVtxCount; i < iVtxCount + iAllVtxCount; ++i)
			{
				pSys[i].vPos = container->vecPos[i - iAllVtxCount];

				// max : three uv 
				pSys[i].vUV0 = container->vecUVs[0][i - iAllVtxCount];
				pSys[i].vUV1 = container->vecUVs[1][i - iAllVtxCount];
				pSys[i].vUV2 = container->vecUVs[2][i - iAllVtxCount];

				pSys[i].vColor = Vec4(1.f, 0.f, 1.f, 1.f);
				pSys[i].vNormal = container->vecNormal[i - iAllVtxCount];
				pSys[i].vTangent = container->vecTangent[i - iAllVtxCount];
				pSys[i].vBinormal = container->vecBinormal[i - iAllVtxCount];
				pSys[i].vWeights = container->vecWeights[i - iAllVtxCount];
				pSys[i].vIndices = container->vecIndices[i - iAllVtxCount];
			}
			...
			for (size_t i = 0; i < container->vecIdx[0].size(); ++i)
			{
				container->vecIdx[0][i] += iAllVtxCount;
			}
...
int CMesh::Load(const wstring& _strFilePath)
{
	...
	m_tVBDesc = tDesc;
	m_iVtxCount = m_tVBDesc.ByteWidth / sizeof(Vtx);
