#ifndef _LINK_PARTICLE_SPARK
#define _LINK_PARTICLE_SPARK
...
void CS_ParticleSpark(uint3 _id : SV_DispatchThreadID)
{
    ...
    if (0 == Particle.iActive)
    {
        ...
        if (Particle.iActive)
        {
            ...
            if( Option & SPREAD_XY )
            {
                Particle.vRelativePos.xy = float2(cos(fTheta), sin(fTheta)) *  fSpawnRange;
            }
            else
            {
                Particle.vRelativePos.xyz = float3(vRandom.x - 0.5f, vRandom.y - 0.5f, vRandom.z - 0.5f) * fSpawnRange;
            }
            
            
            
            Particle.vDir.xyz = normalize(Particle.vRelativePos.xyz);
